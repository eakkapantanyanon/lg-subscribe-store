...
check(/catalog\.css\?v=14/.test(catalogHtml) && /\.catalog-grid \.p-model \{[\s\S]*?color: #6f666a/.test(fs.readFileSync(path.join(ROOT, 'catalog.css'), 'utf8')), 'Catalog model text ใช้ contrast ที่ผ่าน WCAG และ cache version ล่าสุด');
...
check(/พร้อมเลือกแล้ว ไปต่อได้ 3 ทาง/.test(guide) && /href="products\.html">เลือกสินค้าตามงบ/.test(guide) && /href="promotions\.html#official-campaigns">ดูโปรโมชั่นเดือนนี้/.test(guide) && /line\.me\/R\/ti\/p\/@lgthailand/.test(guide), 'Sales guide มี CTA ปิดท้ายไป Catalog, Promotions และ LINE LG Thailand ครบ');
