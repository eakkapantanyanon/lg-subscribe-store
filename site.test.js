...
check(/catalog\.css\?v=12/.test(catalogHtml) && /\.catalog-grid \.p-model \{[\s\S]*?color: #6f666a/.test(fs.readFileSync(path.join(ROOT, 'catalog.css'), 'utf8')), 'Catalog model text ใช้ contrast ที่ผ่าน WCAG และ cache version ล่าสุด');
...
check(catalogSource.includes('class="p-emoji" aria-hidden="true" hidden') && catalogSource.includes('image.nextElementSibling') && catalogSource.includes('emoji.hidden = false'), 'Catalog fallback emoji stays hidden for valid product images and appears only after image failure');
