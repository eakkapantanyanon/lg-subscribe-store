...
check(/\.catalog-brand small \{[^}]*font-size: 11px; letter-spacing: \.08em;/.test(catalogCss), 'Catalog brand subtitle มีขนาดและ tracking ที่อ่านง่าย');
...
check(/\.pdp-top \.brand small \{ display: block; letter-spacing: \.1em; font-size: 11px/.test(pdp) && /\.summary-trust span \{ color: var\(--muted\); font-size: 12px/.test(pdp), 'PDP metadata และ trust copy ไม่เล็กจนอ่านยาก');
check(/aside \.big-label \{ font-size: 12px; line-height: 1\.4; \}/.test(pdp) && /aside \.missing-hint .*font-size: 12px; line-height: 1\.4/.test(pdp), 'PDP mobile summary label และคำแนะนำมีขนาดอ่านง่าย');
...
check(/calculator-core\.js/.test(salesToolkit) && /id="promoStatus"/.test(salesToolkit) && /LG\.parseBEDate/.test(salesToolkit) && /ใช้งานอยู่/.test(salesToolkit) && /หมดอายุ/.test(salesToolkit), 'Sales Toolkit Home แสดงช่วงวันที่แบบอ่านง่ายและสถานะแคมเปญจาก Promotion Config');
...
check(/id="productSearch"/.test(quotation) && /visibleProductIndexes/.test(quotation) && /searchHint\.textContent='พบ '/.test(quotation), 'Quotation Phase 2 ค้นหารุ่น ชื่อ และหมวดสินค้าได้');
check(/addBtn\.disabled=!visibleProductIndexes\.length/.test(quotation) && /ไม่พบสินค้าที่ตรงกับการค้นหา/.test(quotation) && /pls\.disabled=false/.test(quotation), 'Quotation ปิดปุ่มเพิ่มและคืนสถานะแพ็กเกจถูกต้องเมื่อค้นหาไม่พบ/กลับมาพบสินค้า');
...
check(/id="emptyPreview"/.test(productCard) && /function setProductEmptyState/.test(productCard) && /copyTextBtn'\)\.disabled=empty/.test(productCard) && /lineText\.value=''/.test(productCard), 'Product Card ไม่ค้าง Preview/ข้อความรุ่นก่อนเมื่อค้นหาไม่พบสินค้า');
...
check(/<script src="analytics\.js\?v=ga4-4" defer><\/script>/.test(home) && /<script src="products\.js" defer><\/script>/.test(home), 'Home defer analytics และ canonical products.js โดยไม่ block initial render');
...
check(/funnel_session_id/.test(analyticsSource) && /funnel_stage/.test(analyticsSource) && /funnel_stage_view/.test(analyticsSource) && /home: 'awareness'/.test(analyticsSource) && /promotions: 'promotion'/.test(analyticsSource) && /guide: 'education'/.test(analyticsSource) && /catalog: 'discovery'/.test(analyticsSource) && /'product-detail': 'consideration'/.test(analyticsSource) && /cart: 'lead_handoff'/.test(analyticsSource) && /guide_smart_finder_click/.test(analyticsSource) && /guide_conversion_click/.test(analyticsSource) && /data-page="guide"/.test(guide), 'Analytics ครอบคลุม funnel ตั้งแต่ Home/Promotion → Discovery → PDP → Lead handoff และวัด CTA หลักของ Guide ด้วย session-scoped context โดยไม่เก็บ PII');
...
check(/event_schema_version: 1/.test(analyticsSource) && /event_scope: 'lg_subscribe_store'/.test(analyticsSource), 'Analytics ทุก custom event มี schema version และ scope กลางสำหรับทำรายงานอย่างเสถียร');
check(/add_to_cart_click/.test(analyticsSource) && /track\('add_to_cart'/.test(pdp) && /lead_order_copied/.test(cart) && /lead_line_open/.test(cart), 'Conversion events แยก intent → success → lead handoff ชัดเจน ไม่ใช้ event เดียวปนหลายความหมาย');
...
check([home, pdp, catalogHtml, promotions, guide, cart].every((page) => /analytics\.js\?v=ga4-4/.test(page)), 'ทุก flow หลักใช้ cache-busted GA4 analytics bundle เวอร์ชันเดียวกัน');
