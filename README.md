...
| `service-cycles.js` | รอบบริการที่ตรวจสอบจาก Price List; ไม่ใช่รายละเอียดงาน/อะไหล่ |
| `docs/audits/care-service-source-register.md` | ทะเบียน Source of Truth สำหรับรายละเอียด Care Service เฉพาะรุ่น |
| `scripts/audit-care-service.js` | สร้าง `docs/audits/care-service-coverage.json` ว่ารุ่นใดยืนยันรอบบริการแล้ว และรุ่นใดยังรอรายละเอียด Care Service |
...
2. **PDP** (`product.html`) — gallery (รูปใหญ่ + thumbnail) ฝั่งซ้าย · ฝั่งขวา: ชื่อ/รหัสรุ่น/รีวิว/คุณสมบัติ/สเปกเต็ม
   - **เลือกประเภทของคุณ**: ข้อมูลลูกค้า (เดิม/ใหม่) · ประเภทแผน (เช่ารายเดือน/ซื้อขาด — เฉพาะสินค้าที่มี) · ประเภทการดูแล (พร้อมบริการ/เปลี่ยนเอง/ไม่รับบริการ + ปุ่มรายละเอียดบริการ) · ระยะเวลาสัญญา (โชว์ราคา/เดือนในตัวการ์ด)
...
3. รัน `node scripts/audit-care-service.js` และ regression tests ก่อนเผยแพร่; หน้า HTML โหลด `products.js` โดยตรง ไม่ต้อง re-inline dataset
...
node pdp.test.js
node site.test.js
node scripts/audit-care-service.js
```
